...
class NSP_GK5_View {
	...
	static function header($config, $item) {
		...
		if($config['news_header_link'] == 1) {
			$link_attrs = ' title="Czytaj więcej o: '.htmlspecialchars($item['title']).'"';
			...
			$output_html = '<h4 class="nspHeader'.$class.'" title="Czytaj więcej o: '.htmlspecialchars($item['title']).'">';
			$output_html .= $output;
			$output_html .= '</h4>';
	...
	static function lists($config, $item, $num) {
		...
		if($config['list_title_limit'] > 0) {
			...
			if(JString::strlen($title) > 0) {
				$title_attrs  = 'title="Czytaj więcej o: '.htmlspecialchars($item['title']).'"';
